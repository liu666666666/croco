%==============================================================
% BAROCLINIC CONVERSION TERM OF SPECTRAL KE BUDGET
...
%
  if loadmode==1,
    eval(['!cp ',root,specname,'_KE_C_l',num2str(ilevel),'.mat spectrum_KE.mat'])
    ...
    for kchoice=klist,  %%%%%%%%%%%%%%%
      ...
      [amp,count,ktmp,dk]=compute_spectral_flux_wb(ctl,model,dx,kchoice,window,lims,lit,ilevel);
      ampI=ampI+cffz*amp;
    end %kchoice
    amp=ampI;
    save spectrum_KE amp count ktmp dk
    eval(['!mv spectrum_KE.mat ',root,specname,'_KE_C_l',num2str(ilevel),'.mat'])
  end
...
ylabel(['KE conversion [10^{-',num2str(expon),'} m^3/s^3]'])
set(gca,'fontsize',16)
%
