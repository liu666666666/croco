...
for iit=1:length(lit)
  ...
  if constant_Kv,
    Kv=1.e-4*ones(size(dzw));
  else
    Kv=rnt_loadvar_partialz(ctl,it,'AKv',kmin,kmax);      % Kv is 3d
    Kv=squeeze(Kv(Lmin:Lmax,Mmin:Mmax,:));
  end
%
...
%
  ...
  if kchoice==N,
    ...
    dissu=[Kv(:,:,k+1).*(u(:,:,k+1)-u(:,:,k  ))./dzw(:,:,k+1) - ...
           Kv(:,:,k  ).*(u(:,:,k  )-u(:,:,k-1))./dzw(:,:,k  )] ...
                                               ./dzr(:,:,k);
    dissv=[Kv(:,:,k+1).*(v(:,:,k+1)-v(:,:,k  ))./dzw(:,:,k+1) - ...
           Kv(:,:,k  ).*(v(:,:,k  )-v(:,:,k-1))./dzw(:,:,k  )] ...
                                               ./dzr(:,:,k);
  end
%
